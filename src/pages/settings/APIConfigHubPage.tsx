  ...
  const loadData = async () => {
    ...
    if (hId) {
      ...
      const [aiRes, apiRes, logsRes] = await Promise.all([
        ...
        supabase.from("ai_feature_logs").select("feature_key, tokens_used, success, created_at").eq("hospital_id", hId).gte("created_at", sevenDaysAgo.toISOString()),
      ]);
      ...
      for (const log of (logsRes.data || [])) {
        if (!agg.has(log.feature_key)) agg.set(log.feature_key, { calls: 0, tokens: 0, errors: 0, last_used: log.created_at || "" });
        const entry = agg.get(log.feature_key)!;
        entry.calls++;
        entry.tokens += log.tokens_used || 0;
        if (!log.success) entry.errors++;
        if (log.created_at && log.created_at > entry.last_used) entry.last_used = log.created_at;
      }
    ...
    <SettingsPageWrapper title="API Configuration Hub" hideSave>
      <div className="max-w-[960px] mx-auto space-y-8">
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-4">Live usage across all AI features for this hospital</p>
          {usageLogs.length === 0 ? (
