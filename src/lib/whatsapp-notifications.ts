...
For queries: ${opts.hospitalPhone || "Contact hospital lab"}`;
  ...
  const templateName = await shouldAutoSend(opts.hospitalId, "lab_result_ready");
  if (templateName) {
    await sendWATITemplate({
      hospitalId: opts.hospitalId,
      phone: opts.phone,
...
For emergencies: ${opts.hospitalPhone || "Contact hospital"}`;
  ...
  const templateName = await shouldAutoSend(opts.hospitalId, "discharge_summary");
  if (templateName) {
    await sendWATITemplate({
      hospitalId: opts.hospitalId,
      phone: opts.phone,
...
${opts.hospitalName}`;
  ...
  const waUrl = makeWaUrl(opts.phone, msg);
  const templateName = await shouldAutoSend(opts.hospitalId, "prom_survey");
  if (templateName) {
    await sendWATITemplate({
      hospitalId: opts.hospitalId,
      phone: opts.phone,
...
— Aumrti HMS (${opts.hospitalName || "Hospital"})`;
  ...
  const waUrl = makeWaUrl(opts.phone, msg);
  const templateName = await shouldAutoSend(opts.hospitalId, "staffing_alert");
  if (templateName) {
    await sendWATITemplate({
      hospitalId: opts.hospitalId,
      phone: opts.phone,
